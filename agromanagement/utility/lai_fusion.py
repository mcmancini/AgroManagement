...
class LAI_S1S2_fusion() :
	...
	--------------------------------------
		...
		for i in range(len(folds)):
			...
			""" %('%s'%(self.s1dir),folds[i],poly,'%s'%(self.s1dir),folds[i][:-4])
	...
	def S2_to_LAI(self) : 
		...
		*** The final LAI maps' spatial resolution can be adjusted by changing the <targetResolution> (line 332) -- currently 40m 
		...
		for i in range(len(folds)):
			...
			""" %(self.s2dir,folds[i],'%s'%(self.s2dir),folds[i][:-4])
		...
		os.chdir("%sprocessed" %(self.s2dir))
		folders = []
	...
	def lai_ts_creation(self) : 
		...
		bbox_splitter = BBoxSplitter([polygon], 4326, (5,5))  # bounding box will be split into x-times-x bounding boxes
